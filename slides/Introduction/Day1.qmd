...
## Agenda {data-background="./_extensions/emc/www/trianglelb.svg" class="center"}
...
## Where do I get R ? {.triangledb}
  ...
  - choose your platform, e.g., Windows, Linux
    - e.g., for Windows: Windows $\rightarrow$ base $\rightarrow$ Download R 4.4.0 for Windows
    - Install 

  
## How does R work ? {.triangledb}
...
* Default Output is not so nice looking (but wait ultil Day 4)
...
* By default uses only one CPU at a time for most tasks (can be changed using packages)
...
<h3 syle="margin-left:1em">&emsp;How to use</h3>
...
# perhaps a slide about using R as a calculator
```
...
* When transforming and analyzing data we often need to **select** specific [**observations**]{color="#afcc46"} or [**variables**]{color="#e84720"}
  - Examples: Select ...
      ...
      * [the age for male patients or patients that have]{style="font-size: 90%;"} serum bi[lirubin > 3]{style="font-size: 90%;"}
